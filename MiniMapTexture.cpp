	...
	Renderer::Backend::IDeviceCommandContext* deviceCommandContext, const CTerrain* terrain)
{
	...
	for (size_t i = 0; i < textureSize * textureSize; ++i)
		texData[i] = 0xFFFF0000; // 0xAABBGGRR
	...
	const CTerrain* terrain)
{
	...
	for (u32 j = 0; j < height; ++j)
	{
		...
		for (u32 i = 0; i < width; ++i)
		{
			...
			{
				// shallow water
				*dataPtr++ = 0xff001000; //colour of coastline
			}
			else if (avgHeight < m_WaterHeight)
			{
				// Set water as constant color for consistency on different maps
				*dataPtr++ = 0xFFB4A17E; //colour of ocean
			}
			else
			{
				*dataPtr++ = 0xFF86B9E7; //colour of landmass
			}
		}
	}
	...
	Renderer::Backend::IDeviceCommandContext* deviceCommandContext)
{
	...
	if (doUpdate)
	{
		...
		//CFG_GET_VAL("gui.session.minimap.icons.enabled", iconsEnabled); icons are buildings
		...
		for (CSimulation2::InterfaceList::const_iterator it = ents.begin(); it != ents.end(); ++it)
		{
			...
			if (cmpMinimap->GetRenderData(v.r, v.g, v.b, posX, posZ))
			{
				...
				if (vis != LosVisibility::HIDDEN)
				{
					...
					{
						//if ((*attrColor)[0] == 69 && (*attrColor)[1] == 107)
						{
							AddEntity(v, attrColor, attrPos, entityRadius); // Add ntities to plot
							++m_EntitiesDrawn;
						}
					}
					...
					if (m_Icons.size() < MAX_ICON_COUNT)
					{
						...
						const CColor color(1.0f, 0.0f, 0.0f, iconsOpacity); // Town centers and buildings
	...
	if (m_EntitiesDrawn > 0)
	{
		...
		deviceCommandContext->DrawIndexed(m_IndexArray.GetOffset(), m_EntitiesDrawn * 6, 0); // Draw All map Entities
